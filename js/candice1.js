...
d3.select('select').on('change', () => {
    ...
    var index = parseInt(d3.select(this).property('value'));
...
function trace_function(item) {
    ...
    var layout = {
        title: (`${names[0]}\'s Birthday Week - Forecasted vs. Actual Crime`),
        xaxis: { title: "Time" },
        yaxis: {
            title: "Levels of Crime",
            type: "linear"
